    ...
    <>
      <div class="relative">
        <div
          ...
          class={`h-full w-full hover:cursor-grab ${stateStyles[state().type] ?? ''}`}
        >
          {props.children || (
            <div class="flex items-center rounded p-1 text-sm hover:bg-slate-100">
              <RiEditorDraggable />
              <span class="shrink grow truncate">{props.content}</span>
            </div>
          )}
        </div>
